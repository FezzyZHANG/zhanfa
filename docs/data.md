...
store = Store()                         # 使用 DATA_DIR；未配置时为 data/
custom_store = Store(base_dir="data")  # 显式目录
...
`POST /api/data/initialize` 获取股票列表后，会从同一个 `Fetcher.store` 导入关系数据库。因此设置 `DATA_DIR` 时，抓取、parquet 缓存与初始化导入始终使用同一目录，不会回退读取工作区默认 `data/`。
