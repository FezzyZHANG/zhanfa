...
- [环境变量清单](environment.md): 所有可配置环境变量及其默认值。
- [发布前检查清单](release-checklist.md): 每次发布前的逐项核对表。
- `auto-code-review/report_full_20260506.md`: 2026-05-06 工程中台全量代码审查报告。
- `auto-code-review/report_full_20260507.md`: 2026-05-07 全量代码审查报告（第二轮），新增 21 项问题，含 6 项 Critical。
...
- 持续维护型工单不使用 `✅ 已完成`；状态写为 `🔁 持续维护（vNNN）`。`NNN` 取最近一次完成检查时已纳入检查的最高工单编号，新增工单本身不会自动推进版本。
...
| [046]  | 前端 BacktestResult 映射函数去重 | P2 - 中 | ✅ 已完成 | 034, 036 |
| [047]  | StockInfo 前后端契约对齐 | P2 - 中 | ✅ 已完成 | - |
| [048]  | get_watchlist_quotes 批量行情查询性能优化 | P3 - 低 | ✅ 已完成 | 043 |
| [049]  | 行业比较接口逐股抓取财务数据导致响应慢 | P3 - 低 | ✅ 已完成 | - |
| [050]  | 回测结果持久化失败缺少日志与失败语义 | P2 - 中 | ✅ 已完成 | 034 |
| [051]  | 回测请求日期格式统一由后端校验转换 | P2 - 中 | ✅ 已完成 | - |
| [052]  | 调度时间配置与实际注册值对齐 | P2 - 中 | ✅ 已完成 | 041 |
| [053]  | 策略 code_ref fallback 与注册源统一 | P3 - 低 | ✅ 已完成 | - |
| [054]  | 补齐关键链路与降级场景测试 | P2 - 中 | ✅ 已完成 | 041, 043 |
| [055]  | Docker 前端等待后端健康后再暴露服务 | P3 - 低 | ✅ 已完成 | - |
| [056]  | pre-commit 补齐本地类型与前端质量门禁 | P4 - 低 | ✅ 已完成 | - |
| [057]  | 补齐发布清单与环境变量文档 | P4 - 低 | ✅ 已完成 | - |
| [058]  | 修复 CI lint 错误（后端 ruff + 前端 ESLint） | P1 - 高 | ✅ 已完成 | - |
| [042a] | 恢复 max_age TTL 缓存命中测试 | P2 - 中 | ✅ 已完成 | 041 |
| [059]  | 代码审查 Critical/High 后端修复 | P0 - 紧急 | ✅ 已完成 | - |
| [060]  | 代码审查 Critical/High 前端修复 | P0 - 紧急 | ✅ 已完成 | - |
| [061]  | 代码审查 基础设施修复 | P0 - 紧急 | ✅ 已完成 | - |
| [062]  | 代码审查 代码质量改进 | P1 - 高 | ✅ 已完成 | - |
| [063]  | 代码审查 测试覆盖补齐 | P2 - 中 | ✅ 已完成 | - |
| [064]  | akshare 默认不继承系统代理 | P1 - 高 | ✅ 已完成 | - |
| [065]  | 引入腾讯行情 Provider 并切换日线主源 | P1 - 高 | ✅ 已完成 | 043, 064 |
| [066]  | 关键用户场景测试与跨端契约持续维护 | P1 - 高 | 🔁 持续维护（v066） | 009, 019, 032, 054 |
| [067]  | 跨端用户场景测试框架与契约门禁 | P1 - 高 | 📋 待开始 | 066, 009, 032 |
...
- ⛔ 阻塞 — 等待外部依赖
- ❌ 取消 — 不再需要
- 🔁 持续维护（vNNN）— 长期滚动检查；版本号表示最近一次检查覆盖到的最高工单编号
...
[059]: tickets/TICKET-059.md
[060]: tickets/TICKET-060.md
[061]: tickets/TICKET-061.md
[062]: tickets/TICKET-062.md
[063]: tickets/TICKET-063.md
[064]: tickets/TICKET-064.md
[065]: tickets/TICKET-065.md
[066]: tickets/TICKET-066.md
[067]: tickets/TICKET-067.md
