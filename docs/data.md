...
```
akshare API → Fetcher (标准化) → Store (parquet 缓存)
            → Pipeline (清洗/对齐/特征)
            → Strategy / Backtest 消费
```
...
返回的 DataFrame 列名为英文：`open, high, low, close, volume, amount, turnover`，index 为 `DatetimeIndex`。
...
过期后 `load()` 返回 `None`，触发 Fetcher 重新拉取并覆盖缓存。

### 缓存完整性校验

- `stock_list()`: 加载缓存后检查行数 ≥5000，防止截断数据。不满足时自动删除坏缓存并重新拉取。
- `daily()`: 加载缓存后检查行数 ≥1，空缓存自动修复。
...
在 `fetcher.py` 中新增方法，遵循三个约定：
1. 优先 `self.store.load()` 读缓存
2. 获取原始数据后标准化（中文列名→英文、中文单位→float）
3. `self.store.save()` 缓存结果
...
查询单只股票的缓存状态：日线有无/起止日期/行数、财务数据有无/起止日期/行数、所属自选股组。
