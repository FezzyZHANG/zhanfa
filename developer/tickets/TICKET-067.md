...
- 首版不补齐数据刷新、自选股、回测等全部业务旅程；完成后的首轮迭代仅补数据管理成功主旅程，失败/部分失败、自选股和回测仍由 `TICKET-066` 维护。
...
- [x] smoke/scenario 的浏览器请求真实到达 FastAPI；从空状态初始化 Fixture 股票列表，再由 Fixture Daily Provider 写入隔离存储，全过程不访问真实外部 Provider。
...
- 将只读取预置统计的 smoke 升级为 `@smoke @scenario` 数据生命周期：空状态 → 初始化 → Fixture 日线抓取 → 临时 parquet 与 SQLite 状态更新 → 页面重载持久化。
- Fixture 运行时除 Daily Provider 外同时替换股票列表外部边界；`test:e2e:scenario` 不再用 `--pass-with-no-tests` 占位。
- 场景发现 `/api/data/initialize` 的 `import_stocks()` 仍使用默认 `data/`，会绕过 `Fetcher` 的 `DATA_DIR`。接口现复用 `fetcher.store`，并新增 API 回归测试，保证自定义数据目录和 E2E 临时目录使用同一缓存源。
- 强制刷新弹窗移除硬编码 `akshare`，改为“当前配置的数据源”；该用户可见契约由 Playwright 场景覆盖。
...
- `npm run test:e2e:scenario`：1 passed；真实调用 `/api/data/initialize` 与 `/api/data/refresh`，返回 Fixture Provider，统计从 0 更新为 1 个日线缓存 / 4 个总缓存行，页面重载后状态仍在。
- 首轮场景按预期失败并生成诊断产物，定位到初始化读取默认工作区缓存；修复为 `import_stocks(store=fetcher.store)` 后通过。
- `uv sync --dev` 与 `npm ci`：通过；前端干净安装 448 packages，仍报告 6 个既有依赖漏洞（1 low / 2 moderate / 3 high），本轮未扩大依赖范围。
- `uv run ruff check src/`、`uv run mypy src/`：通过，mypy 检查 52 个 source files。
- `uv run pytest -v`：390 passed，233 warnings；新增初始化 Store 身份回归通过。
- `npm run lint`、`npm run test`、`npm run build`：通过；Vitest 15 files / 169 tests，构建仅保留既有 chunk size warning。
- `npm run contract:check`：OpenAPI 固定产物与前端生成类型均无漂移。
- `npm run test:e2e`：1 passed；成功/失败运行均清理临时 runtime，工作区 `data/` 快照未变化。

## 残余风险

- 本地已按新增 job 的命令与隔离语义验证；远端 Linux job 需在分支首次推送后由 GitHub Actions 实际确认。
- 数据刷新失败/部分失败与 `deferred` 展示、自选股、回测和真实腾讯定时探针继续由 `TICKET-066` 滚动维护；本轮基线已推进为 `v067`。
