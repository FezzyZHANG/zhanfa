...
### 后端
- [ ] 移除所有未使用的导入（可直接 `ruff check --fix src/`）
- [ ] 修复 `data.py` 导入顺序：将 `logger = logging.getLogger(__name__)` 移至所有 import 之后
- [ ] 修复 `watchlist_service.py` F821：导入 `Store`（TYPE_CHECKING 或顶层导入）
- [ ] 处理 `jq/adapter.py` F841：删除或补充 `has_rsi`/`has_ma` 变量逻辑
- [ ] 验证 `ruff check src/` 零错误

### 前端
- [ ] 提取 `StockDataTable.tsx` 的 `SortIcon` 为文件级组件
- [ ] 修复 `StockDetailPage.tsx` render 期间 ref 访问：`containerWidth` 和 `mainTimeScale` 改用 state
- [ ] 修复 `KlineChart.tsx` useEffect 缺少 `ref` 依赖
- [ ] 修复 `StrategiesPage.tsx` useMemo 依赖不稳定
- [ ] 验证 `npm run lint` 零错误
